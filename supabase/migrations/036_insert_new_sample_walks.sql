-- Insert new sample walks for all users (idempotent - can be run multiple times)
-- This version deletes old sample walks first, then inserts new ones

-- Helper function to generate route coordinates
CREATE OR REPLACE FUNCTION public.generate_sample_route(
  start_lat DECIMAL,
  start_lng DECIMAL,
...
BEGIN
  -- Delete existing sample walks first (to ensure fresh data)
  ...
  FOR user_record IN SELECT id FROM public.user_profiles LOOP
    ...
    RAISE NOTICE 'Created sample walks for user: %', user_record.id;
  END LOOP;
  
  RAISE NOTICE 'Sample walks creation completed';
END;
$$;
