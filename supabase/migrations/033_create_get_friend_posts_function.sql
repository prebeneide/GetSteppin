-- Function to get feed posts from friends
-- Returns posts from friends with post details, walk info, and engagement stats
...
BEGIN
  ...
  WITH friend_posts AS (
    ...
    INNER JOIN public.friendships f ON (
      (f.user_id = user_id_param AND f.friend_id = p.user_id AND f.status = 'accepted') OR
      (f.friend_id = user_id_param AND f.user_id = p.user_id AND f.status = 'accepted')
    )
